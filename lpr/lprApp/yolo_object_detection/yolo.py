...
def process_image(image):
	name = image.upload.url.split("/")[-1]
	module_dir = os.path.dirname(__file__)
	
	yolo_folder_name = os.path.join(module_dir, 'yolo_coco')
	print(BASE_DIR)
	print(yolo_folder_name)
	image_file_location = str(BASE_DIR) + image.upload.url
	# os.path.join(BASE_DIR,image.upload.url)
	...
	# yolo --- location of yolo-coco
	# image --- location of image file
	weightsPath = os.path.sep.join([yolo_folder_name, "car_gpu_final.weights"])
	configPath = os.path.sep.join([yolo_folder_name, "car.cfg"])
	...
	cv2.imwrite(str(MEDIA_ROOT) +"/images/"+ name, img)
